...
do
    ...
    then
        echo "===================================================================================="
        echo "Turing Machine Emulator"
        echo "Script allows to visualize Turing Machine working proccess"
        echo "===================================================================================="
        echo "Usage: turing-machine.sh <file> <tape_data> [options]"
        echo "Available options:"
        echo "--help print this page"
        echo "-s <speed_value_ms> set screen refresh rate"
        echo "If speed_value is not supplied, default value will be set to 0.1s"
        echo "------------------------------------------------------------------------------------"
        ...
        echo "Each command should be written in a newline"
...
then
    ...
    echo "Usage: turing-machine.sh [file] [tape_data] -s <speed_value_ms>"
...
do
    ...
    then
        ...
        then
            ...
            then
                echo -e "Infinite loop detected: \"$state,$charset,$action,$switch\" Exiting...\n"
                exit 1
            fi
            tape[$head]=$action
        else
        ...
        output+="Cells used       :  $(( $rtape -$ltape + 1))\n"
        output+="Input length     :  $length                 \n"
        output+="Performed actions:  $performed              \n"
   	...
   	for (( counter=$ltape; counter<=rtape; counter++))
        ...
        do
            if [ $counter -eq $head ]
            then
                output+="\033[37;1;41m${tape[$counter]}\033[0m"
            else
                output+="${tape[$counter]}"
            fi
        done
   	...
   	for (( counter=$ltape; counter<=rtape; counter++))
        ...
        then
            echo -e "State \"$state\" not found in specified file"
        else
            echo -e "Transition is undefined for state \"$state\" and symbol \"${tape[$head]}\""
        fi
        exit 1
    fi
    clear
